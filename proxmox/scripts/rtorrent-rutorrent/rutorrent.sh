...
# Single mount: HDD_PATH=/mnt/hdd bash rutorrent.sh
# Multiple mounts (space-separated host:container pairs):
...
# NOTE: lxc-attach workaround — build.func fetches the install script from the
# upstream community-scripts repo, which does not have this script yet.
# Remove this block when submitting a PR to community-scripts/ProxmoxVED.
...
if [[ -n "${MOUNT_LIST}" ]]; then
  ...
  for ENTRY in ${MOUNT_LIST}; do
    ...
    if [[ "${CHOWN_MOUNTS}" == "1" ]]; then
      ...
      if [[ "${MP_FS_TYPE}" =~ ^(cifs|nfs|nfs4|smb|fuse\.sshfs)$ ]]; then
        msg_warn "Skipping chown on ${MP_HOST_PATH} (${MP_FS_TYPE}) — set uid=101000,gid=101000 in mount options instead"
      else
        msg_info "Mount mp${MP_INDEX}: setting ownership on ${MP_HOST_PATH} (UID/GID 101000)"
        chown -R 101000:101000 "${MP_HOST_PATH}"
        msg_ok "Ownership set on ${MP_HOST_PATH}"
      fi
    else
      msg_warn "Skipping chown on ${MP_HOST_PATH} — set CHOWN_MOUNTS=1 to enable"
    fi
  ...
  if [[ ${#CONFIGURED_MOUNTS[@]} -gt 0 ]]; then
    ...
    pct stop "${CTID}"
    pct start "${CTID}"
    msg_ok "Container restarted — bind mounts active"
  fi
fi
